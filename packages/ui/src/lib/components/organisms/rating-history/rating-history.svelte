<script lang="ts">
    ...
    const normalizeRating = (value: number) =>
        Math.min(5, Math.max(0, Math.round(value * 2) / 2));
...
<div class="w-full max-w-md rounded-3xl border border-surface-container-low bg-surface-container-lowest p-6 text-on-surface">
    <div class="flex items-center gap-2 text-body2 font-semibold">
