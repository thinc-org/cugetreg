...
<div class="mx-auto w-full max-w-[800px] p-6 md:p-10">
    ...
    {:else if errorMessage || !announcement}
        <div class="flex h-64 items-center justify-center rounded-2xl border border-dashed border-red-200 bg-red-50 text-red-500">
            {errorMessage}
        </div>
    {:else}
        <article class="flex flex-col gap-8">
            <h1 class="text-2xl font-bold leading-tight text-gray-700 md:text-3xl">
                {announcement.title}
            </h1>
